...
#   作者：big log
#   使用说明：
#       该脚本示例需要5台服务器，在八台服务器中有1台是用于执行该脚本的，
#       另外有4台k8s服务器，其他俩台作为load负载均衡服务器。
#
...
#每个节点的IP
export k8s_master01="192.168.88.66"
export k8s_master02="192.168.88.67"
export k8s_master03="192.168.88.68"
export k8s_node01="192.168.88.69"
export k8s_node02="192.168.88.70"
export load_vip="192.168.88.199"

#物理网络ip地址段
export ip_segment="192.168.88.0\/24"
...
export other="k8s-master02 k8s-master03 k8s-node01 k8s-node02 "
...
echo "所需程序已存在"
if (( $filesize == 628551226 && $filesize == 654356480 && $filesize == 556891795)); then
    echo "下载所需程序"
    rm -f $fille_name.tar
    wget $version
else
...
for NODE in $k8s; do scp cni-plugins-linux-amd64* cri-containerd-cni* crictl* "$NODE":/root/ ; done
...
cfssl gencert -ca=/etc/kubernetes/pki/ca.pem -ca-key=/etc/kubernetes/pki/ca-key.pem -config=ca-config.json -hostname=10.96.0.1,$load_vip,127.0.0.1,kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,$domain,$k8s_master01,$k8s_master02,$k8s_master03 -profile=kubernetes   apiserver-csr.json | cfssljson -bare /etc/kubernetes/pki/apiserver
...
kubectl config set-cluster kubernetes --certificate-authority=/etc/kubernetes/pki/ca.pem --embed-certs=true --server=https://$load_vip:8443 --kubeconfig=/etc/kubernetes/controller-manager.kubeconfig
...
kubectl config set-cluster kubernetes --certificate-authority=/etc/kubernetes/pki/ca.pem --embed-certs=true --server=https://$load_vip:8443 --kubeconfig=/etc/kubernetes/scheduler.kubeconfig
...
kubectl config set-cluster kubernetes     --certificate-authority=/etc/kubernetes/pki/ca.pem     --embed-certs=true     --server=https://$load_vip:8443     --kubeconfig=/etc/kubernetes/admin.kubeconfig
...
kubectl config set-cluster kubernetes      --certificate-authority=/etc/kubernetes/pki/ca.pem     --embed-certs=true      --server=https://$load_vip:8443      --kubeconfig=/etc/kubernetes/kube-proxy.kubeconfig
...
for NODE in $Master; do 
    ...
    for FILE in admin.kubeconfig controller-manager.kubeconfig scheduler.kubeconfig kube-proxy.kubeconfig; do 
...
for HOST in $k8s;do
{
    ...
    ssh root@$HOST "mkdir -p /etc/cni/net.d /opt/cni/bin /etc/containerd"
    ssh root@$HOST "tar xf cni-plugins-linux-amd64*.tgz -C /opt/cni/bin/"
    ssh root@$HOST "tar xf cri-containerd-cni*.tar.gz  -C /" 
    ssh root@$HOST "tar xf crictl* -C /usr/bin/"

    echo "配置主机$HOST 配置crictl客户端连接的运行时位置"
    ssh root@$HOST "/etc/systemd/system/containerd.service << EOF 
[Unit]
...
EOF"
    ...
    ssh root@$HOST "/etc/modules-load.d/containerd.conf << EOF 
overlay
br_netfilter
EOF"

    ssh root@$HOST "/etc/sysctl.d/99-kubernetes-cri.conf << EOF 
net.bridge.bridge-nf-call-iptables  = 1
net.ipv4.ip_forward                 = 1
net.bridge.bridge-nf-call-ip6tables = 1
EOF"

    ssh root@$HOST "/etc/crictl.yaml << EOF 
...
EOF"
    ...
    ssh root@$HOST "systemctl restart systemd-modules-load.service"
    ssh root@$HOST "containerd config default | tee /etc/containerd/config.toml"
    ssh root@$HOST "sed -i \"s#SystemdCgroup\ \=\ false#SystemdCgroup\ \=\ true#g\" /etc/containerd/config.toml"
    ssh root@$HOST "sed -i \"s#k8s.gcr.io#registry.cn-hangzhou.aliyuncs.com/google_containers/pause-amd64:3.2#g\" /etc/containerd/config.toml"
    ssh root@$HOST "systemctl enable --now containerd"
...
for HOST in $master01;do
{
    ...
    ssh root@"$HOST" "cd /root/$fille_name/bootstrap ; kubectl config set-cluster kubernetes     --certificate-authority=/etc/kubernetes/pki/ca.pem     --embed-certs=true     --server=https://$load_vip:8443     --kubeconfig=/etc/kubernetes/bootstrap-kubelet.kubeconfig ; kubectl config set-credentials tls-bootstrap-token-user     --token=c8ad9c.2e4d610cf3e7426e --kubeconfig=/etc/kubernetes/bootstrap-kubelet.kubeconfig ; kubectl config set-context tls-bootstrap-token-user@kubernetes     --cluster=kubernetes     --user=tls-bootstrap-token-user     --kubeconfig=/etc/kubernetes/bootstrap-kubelet.kubeconfig ;kubectl config use-context tls-bootstrap-token-user@kubernetes     --kubeconfig=/etc/kubernetes/bootstrap-kubelet.kubeconfig ;mkdir -p /root/.kube ; cp /etc/kubernetes/admin.kubeconfig /root/.kube/config "
...
kubectl -n kubernetes-dashboard create token admin-user > /root/dashboard.txt
EOF
...
echo "配置$HOST ingress"
cat > ingress.sh << EOF 
cd /root/$fille_name/ingress/
kubectl  apply -f deploy.yaml 
kubectl  apply -f backend.yaml 
EOF

scp ingress.sh root@$HOST:

ssh root@"$HOST" "bash -x /root/ingress.sh" 
...
function menu() {
    ...
    echo -e "#                                                               #"
    echo -e "# 该脚本示例需要5台服务器，在5台服务器中有1台是用于执行该脚本的      #"
    echo -e "# 另外有4台k8s服务器，其他俩台作为load负载均衡服务器。                   #"
    ...
    echo "####################################################################"
    echo " -------------"
    echo -e "  ${GREEN}1.${PLAIN}  v1.24.0"
    echo " -------------"
    echo -e "  ${GREEN}2.${PLAIN}  v1.24.1"
    echo " -------------"
    echo -e "  ${GREEN}3.${PLAIN}  v1.24.2"
    echo " -------------"
    echo -e "  ${GREEN}0.${PLAIN}   退出"
    echo 

    read -p " 请选择操作[0-4]：" biglog
    case $biglog in
        0)
            exit 0
            ;;
        1)
            version="https://ghproxy.com/https://github.com/cby-chen/Kubernetes/releases/download/v1.24.0/kubernetes-v1.24.0.tar"
            fille_name="kubernetes-v1.24.0"
            ;;
        2)
            version="https://ghproxy.com/https://github.com/cby-chen/Kubernetes/releases/download/v1.24.1/kubernetes-v1.24.1.tar"
            fille_name="kubernetes-v1.24.1"
            ;;
        3)
            version="https://ghproxy.com/https://github.com/cby-chen/Kubernetes/releases/download/v1.24.2/kubernetes-v1.24.2.tar"
            fille_name="kubernetes-v1.24.2"
            ;;
        *)
